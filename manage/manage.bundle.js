...
async function initialiseAuthentication() {
  ...
  try {
    ...
    const hasIdentityToken = /^#(?:invite_token|recovery_token|confirmation_token)=/.test(
    ...
    message.textContent = user ? "Existing login found." : "No existing login found.";
    if (user) {
      showSuccess(user);
    } else {
      showLogin();
...
// Milestone 3.1 treatment browser
