...
function removeGalleryItem(editor){updateSiteSectionFromForm();const index=Number(editor.dataset.galleryIndex);if(!confirm('Remove this photo from the gallery?'))return;siteSectionWorking.items.splice(index,1);siteSectionWorking.items.forEach((item,order)=>item.order=order);renderSiteSectionEditor();scheduleSiteSectionSave();}
async function uploadGalleryImage(input){const file=input.files?.[0];if(!file)return;if(!['image/jpeg','image/png','image/webp'].includes(file.type)||file.size>5*1024*1024){setSectionStatus('Choose a JPEG, PNG or WebP photo smaller than 5 MB.','error');input.value='';return;}updateSiteSectionFromForm();const editor=input.closest('[data-gallery-index]');const index=Number(editor?.dataset.galleryIndex);input.disabled=true;setSectionStatus('Uploading photo…');try{const body=new FormData();body.append('image',file);const response=await fetch(GALLERY_IMAGE_API,{method:'POST',credentials:'same-origin',body});const result=await response.json().catch(()=>({}));if(response.status===401){lockManager();return;}if(!response.ok)throw new Error(result.error||'Photo upload failed.');siteSectionWorking.items[index]={...siteSectionWorking.items[index],image:result.url};renderSiteSectionEditor();scheduleSiteSectionSave();}catch(error){setSectionStatus(error.message||'Photo upload failed.','error');input.disabled=false;}}
