...
const NEW_TREATMENTS_KEY = 'eb-new-treatments-v1';
const TREATMENT_ORDER_DRAFT_KEY = 'eb-treatment-order-draft-v1';
...
function persistTreatmentOrder(){localStorage.setItem(TREATMENT_ORDER_DRAFT_KEY,JSON.stringify(treatments.map(item=>item.id)));}
...
function addTreatment(){
  ...
  setDraftStatus('✓ New treatment started · hidden until you publish');
}

function moveTreatment(id,direction){
  const currentIndex=treatments.findIndex(item=>item.id===id);
  if(currentIndex<0)return;
  const categoryIndexes=treatments.reduce((indexes,item,index)=>{if(item.category===treatments[currentIndex].category)indexes.push(index);return indexes;},[]);
...
function removeDraft(id){localStorage.removeItem(draftKey(id));if(newTreatmentIds.has(id)){newTreatmentIds.delete(id);treatments=treatments.filter(item=>item.id!==id);persistNewTreatments();}renderTreatments();}
...
document.querySelector('#treatmentList').addEventListener('click',event=>{const orderButton=event.target.closest('[data-treatment-order]');if(orderButton){moveTreatment(orderButton.dataset.treatmentId,orderButton.dataset.treatmentOrder==='up'?-1:1);return;}const row=event.target.closest('.treatment-row-open[data-treatment-id]');if(row)openSummary(row.dataset.treatmentId);});
...
document.querySelector('#discardChangesButton').addEventListener('click',()=>{const treatment=findTreatment(activeTreatmentId);if(!treatment)return;const isNew=newTreatmentIds.has(activeTreatmentId);if(!confirm(isNew?`Discard this new treatment?`:`Discard unpublished changes to ${treatment.title}?`))return;removeDraft(activeTreatmentId);editorDirty=false;if(isNew){activeTreatmentId=null;renderTreatments();showAppView('treatmentsView');setDraftStatus('✓ New treatment discarded');return;}populateEditor(clone(treatment),0);setDraftStatus('✓ Unpublished changes discarded');});
...
const GALLERY_IMAGE_API='/.'+'netlify/functions/gallery-image';
...
function updateGalleryFromForm(){const items=Array.isArray(siteSectionWorking.items)?siteSectionWorking.items:[];siteSectionWorking.items=[...document.querySelectorAll('[data-gallery-index]')].map((editor,order)=>{const current=items[Number(editor.dataset.galleryIndex)]||{};const value=key=>editor.querySelector(`[data-gallery-key="${key}"]`)?.value?.trim()||'';return {...current,title:value('title'),caption:value('caption'),alt:value('alt'),visible:Boolean(editor.querySelector('[data-gallery-key="visible"]')?.checked),order};});}
function updateSiteSectionFromForm(){const def=sectionDefinitions[activeSiteSection];def.fields.filter(field=>!((activeSiteSection==='merchandise'&&field.path==='categories')||(activeSiteSection==='gallery'&&field.path==='items'))).forEach(field=>{const input=document.querySelector(`[data-section-path="${field.path}"]`);setPath(siteSectionWorking,field.path,parseSectionField(field,input.value,getPath(siteSectionWorking,field.path)));});if(activeSiteSection==='merchandise')updateMerchandiseFromForm();if(activeSiteSection==='gallery')updateGalleryFromForm();}
function validateSiteSection(){if(activeSiteSection==='gallery'&&galleryUploadInProgress){setSectionStatus('Please wait for the photo upload to finish.','error');return false;}const invalid=[...document.querySelectorAll('#sectionEditorForm [required]')].find(field=>!field.value.trim());if(invalid){invalid.focus();setSectionStatus('Please complete the highlighted field.','error');return false;}return true;}
async function saveSiteSectionDraft(){clearTimeout(siteSectionSaveTimer);siteSectionSaveTimer=null;if(siteSectionSaving){siteSectionSaveTimer=setTimeout(saveSiteSectionDraft,350);return;}siteSectionSaving=true;updateSiteSectionFromForm();setSectionStatus('Saving online…');try{const response=await fetch(`${SECTION_API}?section=${encodeURIComponent(activeSiteSection)}&draft=1`,{method:'PUT',headers:{'Content-Type':'application/json'},credentials:'same-origin',body:JSON.stringify({content:siteSectionWorking})});const result=await response.json().catch(()=>({}));if(response.status===401){lockManager();return;}if(!response.ok)throw new Error(result.error||'Draft could not be saved.');setSectionStatus(`✓ Saved online · ${relativeTime(result.savedAt)}`,'success');}catch(error){setSectionStatus('Could not save online — your changes remain on this screen.','error');}finally{siteSectionSaving=false;}}
...
function addGalleryItem(){if(activeSiteSection!=='gallery')return;updateSiteSectionFromForm();const items=Array.isArray(siteSectionWorking.items)?siteSectionWorking.items:[];items.push({title:'',caption:'',image:'',alt:'',visible:false,order:items.length});siteSectionWorking.items=items;renderSiteSectionEditor();scheduleSiteSectionSave();document.querySelector(`[data-gallery-index="${items.length-1}"] [data-gallery-key="image"]`)?.focus();}
function changeGalleryOrder(editor,delta){updateSiteSectionFromForm();const index=Number(editor.dataset.galleryIndex);const next=index+delta;const items=siteSectionWorking.items;if(next<0||next>=items.length)return;[items[index],items[next]]=[items[next],items[index]];items.forEach((item,order)=>item.order=order);renderSiteSectionEditor();scheduleSiteSectionSave();}
function galleryImageId(image){try{const url=new URL(image,location.origin);const id=url.pathname==='/.netlify/functions/gallery-image'?url.searchParams.get('id'):null;return /^[a-f0-9-]{36}$/i.test(id||'')?id:null;}catch{return null;}}
function isPublishedGalleryImage(image){return (siteSectionOriginal?.items||[]).some(item=>item.image===image);}
async function deleteDraftGalleryImage(image){const id=galleryImageId(image);if(!id||isPublishedGalleryImage(image))return;await fetch(`${GALLERY_IMAGE_API}?id=${encodeURIComponent(id)}`,{method:'DELETE',credentials:'same-origin'}).catch(()=>{});}
function setGalleryUploadState(value){galleryUploadInProgress=value;document.querySelector('#sectionPublishButton').disabled=value;document.querySelectorAll('#sectionEditorForm [data-gallery-action],#addGalleryItemButton,[data-gallery-key="image"]').forEach(control=>control.disabled=value);}
async function prepareGalleryUpload(file){if(!file.type.startsWith('image/')||file.size>25*1024*1024)throw new Error('Choose an image smaller than 25 MB.');try{const bitmap=await createImageBitmap(file);const scale=Math.min(1,1600/Math.max(bitmap.width,bitmap.height));const canvas=document.createElement('canvas');canvas.width=Math.max(1,Math.round(bitmap.width*scale));canvas.height=Math.max(1,Math.round(bitmap.height*scale));canvas.getContext('2d').drawImage(bitmap,0,0,canvas.width,canvas.height);bitmap.close?.();const blob=await new Promise(resolve=>canvas.toBlob(resolve,'image/jpeg',.86));if(!blob)throw new Error('Photo preparation failed.');return new File([blob],`${file.name.replace(/\.[^.]+$/,'')||'gallery-photo'}.jpg`,{type:'image/jpeg'});}catch(error){if(['image/jpeg','image/png','image/webp'].includes(file.type)&&file.size<=5*1024*1024)return file;throw new Error('This photo could not be prepared. Try saving it as a JPEG and upload again.');}}
async function removeGalleryItem(editor){updateSiteSectionFromForm();const index=Number(editor.dataset.galleryIndex);if(!confirm('Remove this photo from the gallery?'))return;const [removed]=siteSectionWorking.items.splice(index,1);siteSectionWorking.items.forEach((item,order)=>item.order=order);renderSiteSectionEditor();scheduleSiteSectionSave();await deleteDraftGalleryImage(removed?.image);}
async function uploadGalleryImage(input){const file=input.files?.[0];if(!file)return;updateSiteSectionFromForm();const editor=input.closest('[data-gallery-index]');const index=Number(editor?.dataset.galleryIndex);setGalleryUploadState(true);setSectionStatus('Preparing photo…');try{const upload=await prepareGalleryUpload(file);if(upload.size>5*1024*1024)throw new Error('This photo is still too large after optimisation.');const body=new FormData();body.append('image',upload);setSectionStatus('Uploading photo…');const response=await fetch(GALLERY_IMAGE_API,{method:'POST',credentials:'same-origin',body});const result=await response.json().catch(()=>({}));if(response.status===401){lockManager();return;}if(!response.ok)throw new Error(result.error||'Photo upload failed.');const previousImage=siteSectionWorking.items[index]?.image;siteSectionWorking.items[index]={...siteSectionWorking.items[index],image:result.url};renderSiteSectionEditor();setGalleryUploadState(true);scheduleSiteSectionSave();await deleteDraftGalleryImage(previousImage);}catch(error){setSectionStatus(error.message||'Photo upload failed.','error');}finally{setGalleryUploadState(false);}}
...
document.querySelector('#sectionEditorForm').addEventListener('change',event=>{if(event.target.matches('[data-gallery-key="image"]')){uploadGalleryImage(event.target);return;}if(event.target.matches('[data-merchandise-key="image-choice"]')){const editor=event.target.closest('[data-merchandise-index]');const customField=editor?.querySelector('.merchandise-custom-image-field');if(event.target.value==='__custom__'){if(customField)customField.hidden=false;customField?.querySelector('input')?.focus();return;}if(customField)customField.hidden=true;updateSiteSectionFromForm();renderSiteSectionEditor();scheduleSiteSectionSave();}});
document.querySelector('#sectionEditorForm').addEventListener('click',event=>{if(event.target.closest('#addMerchandiseItemButton')){addMerchandiseItem();return;}if(event.target.closest('#addGalleryItemButton')){addGalleryItem();return;}const merchandiseAction=event.target.closest('[data-merchandise-action]');if(merchandiseAction){const editor=merchandiseAction.closest('[data-merchandise-index]');if(merchandiseAction.dataset.merchandiseAction==='up')changeMerchandiseOrder(editor,-1);if(merchandiseAction.dataset.merchandiseAction==='down')changeMerchandiseOrder(editor,1);if(merchandiseAction.dataset.merchandiseAction==='remove')removeMerchandiseItem(editor);return;}const galleryAction=event.target.closest('[data-gallery-action]');if(!galleryAction)return;const editor=galleryAction.closest('[data-gallery-index]');if(galleryAction.dataset.galleryAction==='up')changeGalleryOrder(editor,-1);if(galleryAction.dataset.galleryAction==='down')changeGalleryOrder(editor,1);if(galleryAction.dataset.galleryAction==='remove')removeGalleryItem(editor);});
